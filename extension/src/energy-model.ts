...
export type VampireCategory = 'CPU' | 'Memory' | 'I/O' | 'Algorithmic';

// ─── Calibration State ───────────────────────────────────────────────────────

import { CalibrationFactors } from './benchmarker';

/** Live calibration factors set by the benchmarker on activation */
let _calibration: CalibrationFactors = { cpu: 1, memory: 1, io: 1, algorithmic: 1 };

/**
 * Called by extension.ts after the benchmark completes.
 * All subsequent energy computations will use these factors.
 */
export function setCalibrationFactors(factors: CalibrationFactors): void {
    _calibration = factors;
}

export function getCalibrationFactors(): CalibrationFactors {
    return _calibration;
}
 ...
 */
export function getEffectiveW(staticW: number, category: VampireCategory): number {
    switch (category) {
...
export interface EnergyResult {
    /** Energy wasted in Joules (ΔE) */
    deltaE_joules: number;
    /** CO₂ equivalent in grams (m_CO2) */
    co2_grams: number;
    /** Human-readable ΔE in millijoules */
    deltaE_millijoules: number;
    /** Human-readable CO₂ in micrograms */
    co2_micrograms: number;
    /** Which physical model was applied */
    modelUsed: 'CPU' | 'Memory' | 'I/O' | 'Heuristic';
}

export interface EnergyHit {
    severity: number;
    /** Static energy weight W from the rule definition */
    energyWeight: number;
    /** Calibration-adjusted effective W used in SS formula */
    effectiveW: number;
    deltaE_joules: number;
}
 ...
 */
export function computeCpuDeltaE(deltaC_ratio: number): number {
 ...
 */
export function computeMemoryDeltaE(freqGC_ratio: number): number {
 ...
 */
export function computeIoDeltaE(ioCallMultiplier: number): number {
 ...
 */
export function computeHeuristicDeltaE(energyWeight: number): number {
 ...
 */
export function computeCO2(deltaE_joules: number): number {
...
export function computeEnergyResult(
    ...
    let deltaE: number;
    let modelUsed: EnergyResult['modelUsed'];
 ...
 */
export function computeSustainabilityScore(hits: EnergyHit[], totalLOC: number): number {
    ...
    const weightedSum = hits.reduce(
        (acc, hit) => acc + hit.severity * hit.effectiveW,
        0
    );
